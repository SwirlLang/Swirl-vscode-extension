...
function activate(context: vscode.ExtensionContext) {
    const data = JSON.parse(
        fs.readFileSync(context.asAbsolutePath("src/data.json"), "utf8")
    );

    const keywords: keywords = data.keywords;
    const builtin_funcs: builtin_funcs = data.builtin_funcs;
        ...
        provideDefinition(document, position) {
            ...
            if (builtin_funcs[word] || keywords[word] != undefined) {
                return new vscode.Location(
                    vscode.Uri.file(context.asAbsolutePath(`src/data.json`)),
